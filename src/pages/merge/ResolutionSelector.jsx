...
export default function ResolutionSelector({
  ...
  // const autogenNames = Object.values(individualData)
  //                            .map(data => data.names)
  //                            .map(data => data.filter(name => name.autogeneratedName)[0]);
  ...
  const propertyMap = {
    ...
    autogeneratedName: {
      labelId: 'AutOGEN_NAME',
      getProperty: individualData => deriveIndividualAutogenNameObject(individualData).guid,
      deriveChoices: names => 
        names.map(name => {          
          const label = autogenNames.find(autogen => autogen.guid === name).value;
          return {
            label: label,
            value: name,
          }
